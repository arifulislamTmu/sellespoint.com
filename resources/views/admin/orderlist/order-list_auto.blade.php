<table id="datatable1" class="table display responsive nowrap">
    ...
    <tbody>
      <?php
        $count = 1;
        ?>
      @foreach ($order_list_auto AS $order)
      <tr>
        <td>{{ $count++ }}</td>
        <td>#{{ $order->invoice }}</td>
        ...
        <td>
          ...
          <a href="{{ url('generate-pdf',$order->id) }}" target="_blank" class="btn btn-outline-success btn-sm">
          ...
          <a href="{{ url('/order-hidden',$order->id) }}" class="btn btn-outline-danger btn-sm">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash mt-1" viewBox="0 0 16 16">
